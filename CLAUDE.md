...
- **Three-way documentation sync** — user-facing CLI behaviour is stated in `README.md`, `skills/cflio/SKILL.md`, and the cobra help strings under `internal/cmd/`. Changing the CLI means updating all three in the same PR. Overlaps today, each stated in more than one of the three: the 90 s default `--timeout`; the exit codes; the default `md` output format; the read → edit → update cycle and the sidecar's version lock; the default filenames the two read modes write, and `--markdown`'s output being read-only; the `Degraded` and `Unchecked` counts `read --markdown` reports; `attachments download`'s required `--pattern` and its refusal to overwrite an existing file; and profile resolution.
