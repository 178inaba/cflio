...
- **A short link is a page reference like any other.** `/wiki/x/<token>` links, as Confluence's Share dialog copies them, are decoded locally and work anywhere a page URL does — pass one straight through rather than asking the user for the full URL. (One pointing at a blog post decodes but then 404s, since cflio addresses pages only.)
