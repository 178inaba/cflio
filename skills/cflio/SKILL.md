...
- **A failed reference lookup is reported too, and is a different problem.** `Unchecked: 2 (not looked up; names and links may be missing)` — `unchecked_count` in JSON — means that many mentions or page links could not be checked, because the request failed or there was nothing to query. They render as a bare account ID or page title, exactly like a mention or link that genuinely resolves to nothing, so do not conclude from the rendering that the person or page does not exist. Re-reading without `--markdown` is **not** the fix here: storage resolves no references at all. Run the same `read --markdown` again instead — `cflio` never retries a failed request on its own.
